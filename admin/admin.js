...
/* ---------------- matching ---------------- */
const matchOf = (id) => matches.find((m) => m.menteeId === id || m.mentorId === id) || null;
const statusOf = (id) => { const m = matchOf(id); return m ? (m.status || "in_progress") : "unmatched"; };
function partnerOf(intake) {
  const m = matchOf(intake.id);
  if (!m) return null;
...
function candidatesFor(intake) {
  ...
  return responses.filter((r) => r.role === wantRole)
    .map((r) => {
      const s = scoreCandidate(intake, r);
      return { doc: r, score: s.score, why: s.why, taken: !!matchOf(r.id) };
    })
...
const COLUMNS = [
  ...
  { key: "partner", label: "Matched with", get: (d) => (partnerOf(d) || {}).name || "" },
...
function renderMatches() {
  ...
  $("match-tiles").innerHTML =
    ...
    tile(mentors.filter((r) => statusOf(r.id) === "unmatched").length, "Mentors available");
...
function renderMatchBox(d) {
  ...
  if (p) {
    ...
    box.innerHTML = `
      ...
      <p class="matchbox__who">Matched with <b>${esc(p.name)}</b>
        <span class="matchbox__meta">(${esc(counterpart)})</span></p>
      <p class="matchbox__meta">Date of match: ${esc(p.match.matchedAt || "—")}${
        p.match.notes ? " · " + esc(p.match.notes) : ""}</p>
      <div class="matchbox__row">
        <label class="sr-only" for="mb-status">Match status</label>
        <select id="mb-status">
          ${["in_progress", "matched", "ended"].map((s) =>
            `<option value="${s}"${s === st ? " selected" : ""}>${STATUS_LABEL[s]}</option>`).join("")}
        </select>
        <button class="btn btn--danger btn--sm" id="mb-remove" type="button">Remove match</button>
      </div>`;
    $("mb-status").addEventListener("change", (e) =>
      setStatus(p.match.id, e.target.value).then(() => renderMatchBox(d)));
    $("mb-remove").addEventListener("click", () => removeMatch(p.match.id));
    return;
  }
  ...
  box.innerHTML = `
    ...
    ${cands.length ? `
      ...
      <div class="matchbox__row">
        <button class="btn btn--gold btn--sm" id="mb-create" type="button">Create match</button>
        <span class="hint">Suggestions ordered by <span class="cand-why">AFSC and duty location</span>.</span>
      </div>`
    : `<p class="matchbox__meta">No ${counterpart}s have signed up yet.</p>`}`;

  const create = $("mb-create");
  if (create) create.addEventListener("click", () => {
    const pick = $("mb-pick").value;
    if (!pick) { $("mb-pick").focus(); return; }
      ...
      responses.slice().sort((a, b) => String(b.createdAt || "").localeCompare(String(a.createdAt || "")))
        .map((d) => {
          ...
          return `<tr><td>${fmtDate(d.createdAt)}</td><td>${esc(d.role)}</td>
            ...
            <td>${esc(STATUS_LABEL[statusOf(d.id)])}</td><td>${esc(p ? p.name : "")}</td>
